...
А для того, чтобы посмотреть историю более компактно, используем команду: *git log --oneline*. Это сжатая команда. Но если необходио посмотреть все изменения, то делает *git log*.
